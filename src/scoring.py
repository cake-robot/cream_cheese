...
# --- comeback_margin_q4_close: original flat cap, chosen off the corpus-wide
# distribution of nonzero raw values (min=9, p50=14, p75=17, p90=21, p99=28,
# max=35) -- set near p90 so a typical one-possession-recovery-in-Q4 game
# lands well under 1.0 normalized, while the rarer, more extreme blown-open-
# and-clawed-back-again games clip at the cap rather than dominating the
# composite. SUPERSEDED 2026-09-05 by the non-linear comeback_margin_q4_curve
# below (a flat cap forced choosing between "typical games keep their credit"
# and "extreme games stop tying at 1.0" -- the curve gets both). No longer
# referenced by METRICS; kept as the historical anchor point the curve's own
# control points were tuned against.
...
# --- comeback_margin_q4_close normalization curve (2026-09-05, replacing the
# flat MAX_COMEBACK_MARGIN_Q4 linear cap above) -- a linear cap makes every
# raw value below it lose the SAME proportion of credit no matter where it
# sits, which forced an uncomfortable choice: raising the cap to stop e.g. a
# 20-point and 32-point recovery from scoring identically (both maxed at 1.0)
# also pulled the common case (raw 9-17, ~87% of firing games) down by the
# same flat ratio. Built interactively against the real corpus instead: a
# monotone cubic (PCHIP) interpolation through hand-picked control points, so
# the floor (raw=9, the minimum that can ever fire) drops harder than a
# linear scale would, while a ~20-21 point recovery stays close to "mostly
# there" and anything beyond it keeps adding a small, steadily shrinking
# amount rather than hard-clipping at 1.0. Table is a plain Python list
# (dependency-free at runtime, matching src/wp_baseline.py's/
# src/wp_situational.py's pattern) generated by
# scripts/build_comeback_margin_curve.py -- edit that script's
# CONTROL_POINTS and regenerate, don't hand-edit the table below.
_COMEBACK_MARGIN_Q4_CURVE = [
    0.0000, 0.0243, 0.0496, 0.0759, 0.1031, 0.1311, 0.1599, 0.1893, 0.2194, 0.2500,
    0.2793, 0.3077, 0.3385, 0.3748, 0.4200, 0.4948, 0.5936, 0.6800, 0.7480, 0.8115,
    0.8643, 0.9000, 0.9217, 0.9379, 0.9500, 0.9599, 0.9684, 0.9752, 0.9800, 0.9833,
    0.9860, 0.9882, 0.9900, 0.9917, 0.9934, 0.9949, 0.9963, 0.9976, 0.9986, 0.9994,
    1.0000,
]


def comeback_margin_q4_curve(raw):
    """Maps comeback_margin_q4_close's raw point-margin to a normalized [0,1]
    value via _COMEBACK_MARGIN_Q4_CURVE above. raw is always a non-negative
    int in practice (score differentials are always whole points); anything
    at or past the table's built-in safety margin (40 -- 8 points past the
    corpus's observed max of 32) clamps to 1.0 rather than indexing out of
    range."""
    if raw <= 0:
        return 0.0
    idx = int(raw)
    if idx >= len(_COMEBACK_MARGIN_Q4_CURVE):
        return 1.0
    return _COMEBACK_MARGIN_Q4_CURVE[idx]
...
METRICS = [
    ...
    {"name": "comeback_margin_q4_close", "fn": lambda ctx: comeback_margin_q4_close(ctx["situational_plays"]), "weight": 1.0, "cap": comeback_margin_q4_curve},
]
...
def _normalize(metric_name, raw):
    ...
    cap may be a number (linear raw/cap), None (raw already bounded [0,1]), or
    a callable raw->[0,1] curve (e.g. comeback_margin_q4_curve) -- same three
    forms composite_from() accepts."""
    cap = METRICS_BY_NAME[metric_name]["cap"]
    if callable(cap):
        return min(cap(raw), 1.0)
    if cap is None:
        return min(raw, 1.0)
...
def composite_from(metrics, context):
    ...
    and returns a raw value or None). cap is a number (linear raw/cap), None
    (raw is already bounded [0,1] by construction), or a callable raw->[0,1]
    curve (e.g. comeback_margin_q4_curve) for a metric that needs a
    non-linear normalization shape instead of a flat cap.
