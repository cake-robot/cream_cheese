...
| Metric | Function | Cap |
|--------|----------|-----|
| WP Volatility | `Σ \|WP[i+1] - WP[i]\|` | `MAX_VOLATILITY = 5.0` |
| Lead Changes | crossings of 0.50 | `MAX_LEAD_CHANGES = 10` |
| Time Spent Close | proportion where WP ∈ [0.30, 0.70] | none (already 0–1) |

### Files Changed

- `src/scoring.py` — new module: metric functions, registry, `score_game()`, `score_games()`
- `src/db.py` — add `update_watchability_score(conn, game_id, score)`
- `pipeline.py` — add `--score-only`, `--skip-scoring`, `--rescore` flags; Phase 3 calls
